...
from generator.train_composition_generator import generate_composition
...
def generate_schedule():
    ...
    for route in routes:
        trains_per_route = random.randint(2, 5)
        for train_num in range(trains_per_route):
            [train_composition, train, cars, seats, compositions_cars] = generate_composition()

            compositions.append(train_composition.__dict__)
            ...
            time_slot = ["morning", "midday", "afternoon", "evening", "night"][train_num % 5]
    ...
    print(f"Trains per route: {(routes_processed / len(routes)):.2f}")
    ...
    print("\n\n")
    if len(stations) != len(stations_covered):
        print(f"SKIPPED {len(stations) - len(stations_covered)} STATIONS WITHOUT SCHEDULES")
        print("RERUNNING GENERATOR RECOMMENDED AND/OR INCREASING GENERATED AMOUNT OF PATHS RECOMMENDED.")
